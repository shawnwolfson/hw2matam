...
def addOperation(inventory_dict, name, price, amount):
    sales = 0
    if (price >= 0) or (amount >= 0) or not (name in inventory_dict.keys()):
...
def processInventory(file_name):
...
def find_best_selling_product(file_name):
    ...
    if len(content) == 0:
        return best_seller
    inventory = processInventory(file_name)
    if len(inventory) == 0:
        return best_seller

    best_seller = ['',0]
    for key, value in inventory.items():
        if value[2] > best_seller[1]:
            best_seller[0] = key
            best_seller[1] = value[2]
    for key, value in inventory.items():
        if (value[2] == best_seller[1]) and (key < best_seller[0]):
            best_seller[0] = key
